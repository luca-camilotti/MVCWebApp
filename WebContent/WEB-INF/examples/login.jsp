...
$(document).ready(function(){
	...
	var loginForm = document.getElementById("login-form");	// login form
	...
	$("#reset-password a").click(function () {
		...
		// Check if email field is ok, otherwise return a toast message
		if($('#email').val()=="" || ! $('#email').val().match(/(.*@.*\..*)/)) {
			$('.toast-body').html("Inserisci una email valida!");
			$('.toast').toast('show');
			return;
		}
		
		// 2) Ajax request
		var data = new FormData(loginForm);  // get form data
		
		// inspect form data and print them to browser console
		/*
		for (var pair of data.entries()) {
		    console.log(pair[0]+ ', ' + pair[1]); 
						...
						$.ajax({  // send ajax request to FileUpload servlet
							...
							url : '<%= getServletContext().getContextPath() %>/mail',
							//url : 'http://localhost:7979',
							data : "email="+$("#email").val(), // works only this way!
								
							//mydata, // $("#login-form").serialize(), // data,
							// dataType: "json", 
							processData : false,
							contentType : false,
							cache : false,
...
<form id="login-form" method="post" action="<%= context+"/" %>App/login">
...
<div class="row d-flex justify-content-center">
 ...
 <div class="row d-flex justify-content-center">
  ...
  <div class="row d-flex justify-content-center">
  <div id="reset-password" class="mb-3 col-sm-10 col-md-8 col-lg-4">
  <a href="#" class="link-dark">Reimposta la password</a>
